...
/**
 * Suggestfield_ft class.
 * 
 * Fieldtype will auto suggest previous content saved with this fieldtype
 *
 * @extends EE_Fieldtype
 */
class Suggestfield_ft extends EE_Fieldtype 
{
    public $info = array(
        'name'      => SUGGESTFIELD_NAME,
        'version'   => SUGGESTFIELD_VERSION
    );
    public $EE;
    ...
    public function __construct()
    {
		...
		if (! isset($this->EE->session->cache[SUGGESTFIELD_SHORTNAME])) {
			$this->EE->session->cache[SUGGESTFIELD_SHORTNAME] = array('includes' => array());
		}
		$this->cache =& $this->EE->session->cache[SUGGESTFIELD_SHORTNAME];
    }
    ...
    {    
        ...
        if ($enable_autosuggest === 'yes') {
	    	$this->_include_theme_css('suggestfield.css');
			...
			$attributes['class'] = 'dropdown suggestfield_autosuggest';
    	}
...
JS;
    ...
    public function display_settings($data)
    {
		...
		$this->EE->lang->loadfile('suggestfield');
	...
	{
		if ($db) {
			$query = $this->EE->db->query('SELECT settings FROM exp_fieldtypes WHERE name="suggestfield" LIMIT 1');
			if ($query->num_rows() === 1) {
	...
	private function _theme_url()
	{
		if (! isset($this->cache['theme_url']))
		{
			...
			$this->cache['theme_url'] = $theme_folder_url.'third_party/suggestfield/';
		}
...
// END Suggestfield_ft class

/* End of file ft.suggestfield.php */
/* Location: ./system/expressionengine/third_party/suggestfield/ft.suggestfield.php */